...
3. {ref}`This page <code-processors>` might help if you encounter errors when generating bindings.
 ...
 * You need to look for Ajax links whose markup is an `<a>` the click event is not blocked by Wicket anymore, which will result in a scroll to the top of the page each time the link is clicked. This is always true for Google Chrome, but only if there is a `href` attribute for Firefox. To avoid any kind of trouble, just follow the guidelines detailed {ref}`here <ui-user-actions>`.
...
The external link checker now has its own Maven module. See {ref}`ExternalLinkChecker <external-link-checker>` if you use it in your app.

Related to the new `PropertyService`: you also have to use `JpaExternalLinkCheckerConfig` (import) in your app.

### Properties

Both immutable and mutable properties are now handled by `PropertyService`. See {ref}`PropertyService <property-service>` to use it in your app.
...
See {ref}`PropertyService <property-service>` 
 ...
 * Or (better) use the brand-new HistoryLog framework. See {ref}`HistoryLog & Audit <history-log>`
