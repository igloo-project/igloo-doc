...
* 1.1.28 introduces a breaking change. Configuration system is modified to
...
* BasicApp: Update ``basic`` and ``advanced`` layouts. Revamp sidebar
  (style and positioning) in ``advanced`` layout.
...
* Feedback panel: fix unwanted overlay preventing users to interact with
