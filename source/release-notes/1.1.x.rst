...
1.1.18 (next release, not published)
####################################
...
* Improve inclusion of tables into cards with new custom scss classes (``table-bordered-inner``, ``table-card-body``, ``card-body-table``).
  From now on every content in a ``card`` should be placed under a ``card-body`` element.
* New method ``replaceAll`` from ``CollectionUtils`` utilitary in which we provide the transformation to operate in the reverse collection.
